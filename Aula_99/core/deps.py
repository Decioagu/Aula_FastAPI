...
# Meus módulos
from core.configs import Session, settings, get_session
from core.auth import oauth2_schema
from models.membro_model import MembroModel

# class auxiliar para identificação do id do usuário (MembroModel.id)
class TokenData(BaseModel):
    username: Optional[int] = None

# Dependência (VERIFICAR SENHA E USUÁRIO)
async def get_current_user(db: Session = Depends(get_session), token: str = Depends(oauth2_schema)) -> MembroModel: # type: ignore
    ...
    async with db as session:
        query = select(MembroModel).filter(MembroModel.id == int(token_data.username)) # Filtro
        result = await session.execute(query) # Executar Banco de Dados
        usuario: MembroModel = result.scalars().unique().one_or_none() # Extrair
