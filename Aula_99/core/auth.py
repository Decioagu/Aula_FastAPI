...
async def autenticar(email: EmailStr, senha: str, db: AsyncSession) -> Optional[MembroModel]:
    async with db as session:
        query = select(MembroModel).filter(MembroModel.email == email) # Filtra
        result = await session.execute(query) # Executa Banco de Dados
        usuario: MembroModel = result.scalars().unique().one_or_none() # Extrai
